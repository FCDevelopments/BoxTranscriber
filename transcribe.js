...
const TRANSCRIPT_MARKER = 'Auto-Transcript';
...
// AssemblyAI can then fetch it directly without Box auth.
...
async function transcribeFile(downloadUrl, fileSize) {
  if (fileSize > FFMPEG_THRESHOLD && FFMPEG_PATH) {
    ...
    try {
      ...
      const result = await aai.transcripts.transcribe({ audio_url: uploadUrl, speech_models: ['universal-2'] });
  ...
  const result = await aai.transcripts.transcribe({ audio_url: downloadUrl, speech_models: ['universal-2'] });
  if (result.status === 'error') {
    if (result.error?.includes('no spoken audio')) return null;
    throw new Error(`AssemblyAI: ${result.error}`);
  }
...
async function processFile(file) {
  ...
  const text = await transcribeFile(url, file.size);
  ...
  console.log(`  Transcript ready (${text.length} chars)`);
  const chunks = buildCommentChunks(text);
...
async function main() {
  console.log('============================================');
  console.log('       Box Video Transcriber');
  console.log('============================================');
